<?php
	...
	$param_keys=array('dt', 'user_id', 'service_id', 'rating', 'comment'); // названия полей в таблице БД
	$param_str=array('Дата и время', 'Клиент', 'Филиал', 'Рейтинг', 'Комментарий'); // названия столбцов в таблице для отображения
	$param_ext=array(0, '`users`.`fio`', '`services`.`name`', 0, 0); // поля для select
	$param_need='user_id'; // обязательные поля, без которых не сохранять данные
	$dates=array('dt'); // поля типа "дата"
	$textarea=array('comment'); // поля типа "textarea"
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<?php
	...
	$query="
		...
		WHERE 1
			...
			AND services.id=reviews.service_id
		ORDER BY `$table`.`id`
	";
