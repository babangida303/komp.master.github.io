<?php
	...
	$edit=in_array($_SESSION['level'], array(10, 5, 1));
	$param_keys=array('dt', 'user_id', 'service_id', 'status_id', 'comp', 'repair_id', 'note'); // названия полей в таблице БД
	$param_str=array('Дата и время', 'Клиент', 'Филиал', 'Статус', 'Марка компьютера', 'Тип ремонта', 'Примечание'); // названия столбцов в таблице для отображения
	$param_ext=array(0, '`users`.`fio`', '`services`.`name`', '`statuses`.`name`', 0, '`repairs`.`name`', 0); // поля для select
	$param_need='user_id'; // обязательные поля, без которых не сохранять данные
	$dates=array('dt'); // поля типа "дата"
	$textarea=array('address', 'descr', 'contact', 'products', 'schedule', 'marks'); // поля типа "textarea"
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<?php
	...
	$query="
		SELECT
			...
			$admin_edit
		FROM
			visits, users, services, statuses, repairs
		WHERE 1
			AND visits.user_id=users.id
			AND visits.service_id=services.id
			AND visits.status_id=statuses.id
			AND visits.repair_id=repairs.id
			AND (
				($_SESSION[level]=1 AND visits.user_id=$_SESSION[id]) # клиенту только его визиты
				OR ($_SESSION[level]=10)
			)
		ORDER BY `$table`.`id`
	";
...
<?php
	...
	error_reporting(0);
	for($ind=0; $ind<count($param_keys); $ind++) {
		if ($param_keys[$ind]=='rating') { // spinedit для оценки от 1 до 10
			$buf.='
				<tr>
					<td>'.$param_str[$ind].'</td>
					<td>
						<input id="'.$param_keys[$ind].'" name="'.$param_keys[$ind].'" type="number" min="1" max="10" value="'.$param_values[$param_keys[$ind]].'">
					</td>
				</tr>
			';
		}
		elseif (in_array($param_keys[$ind], $textarea)) { // если это textarea
			$buf.='
...
if ($param_keys[$ind]=='user_id') {
			$query="
				...
				WHERE 1
					AND
						($_SESSION[level]=1 AND id=$_SESSION[id])
						OR ($_SESSION[level]=10)
				ORDER BY fio
			";
};
if ($param_keys[$ind]=='status_id') {
			$query="
				SELECT name AS `name`, `id`
				FROM statuses
				WHERE 1
					AND
						($_SESSION[level]=1 AND id=1) # клиенты могут только поставить забронировано
						OR ($_SESSION[level] IN (10, 5)) # админы и представители могут менять статусы визитов
				ORDER BY name
			";
};
/*
if ($param_keys[$ind]=='worker_id') {
			$query="
				SELECT fio AS `name`, `id`
				FROM users
				WHERE 1
